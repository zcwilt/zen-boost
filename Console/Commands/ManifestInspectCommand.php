...
class ManifestInspectCommand extends AbstractZenBoostCommand
{
    /**
     * @since ZC v3.0.0
     */
    ...
    {
        return 'zen-boost:manifest:inspect';
    }
    ...
    {
        return [
            'bin/zencart zen-boost:manifest:inspect <path>',
            'php zc_cli.php zen-boost:manifest:inspect <path>',
        ];
    }
    ...
    public function handle(ConsoleInput $input, ConsoleOutput $output): int
    {
        ...
        if ($manifestPath === '') {
            $output->errorln('Usage: bin/zencart zen-boost:manifest:inspect <path>');
            return 1;
        }

        $inspector = new \ZenBoostManifestInspector();

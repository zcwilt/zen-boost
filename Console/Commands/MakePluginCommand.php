...
class MakePluginCommand extends AbstractZenBoostCommand
{
    public function getName(): string
    {
        return 'zen-boost:make:plugin';
    }
    ...
    {
        return [
            'bin/zencart zen-boost:make:plugin <unique-key> [version] [--name=<display-name>] [--author=<author>] [--description=<description>]',
            'php zc_cli.php zen-boost:make:plugin <unique-key> [version]',
        ];
    }
    ...
    public function handle(ConsoleInput $input, ConsoleOutput $output): int
    {
        ...
        if ($uniqueKey === '') {
            $output->errorln('Usage: bin/zencart zen-boost:make:plugin <unique-key> [version]');
            return 1;
        }

        $version = trim((string)$input->getArgument(1, 'v1.0.0'));
        $scaffolder = new \ZenBoostPluginScaffolder();

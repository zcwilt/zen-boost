...
class DocsSearchCommand extends AbstractZenBoostCommand
{
    /**
     * @since ZC v3.0.0
     */
    ...
    {
        return [
            'bin/zencart zen-boost:docs:search <terms>',
            'php zc_cli.php zen-boost:docs:search <terms>',
        ];
    }
    ...
    public function handle(ConsoleInput $input, ConsoleOutput $output): int
    {
        ...
        if ($query === '') {
            $output->errorln('Usage: bin/zencart zen-boost:docs:search <terms>');
            return 1;
        }

        $storage = $this->storage();
        $paths = $this->paths();
        $search = new \ZenBoostSearchService();

...
class DocsAskCommand extends AbstractZenBoostCommand
{
    public function getName(): string
    {
        return 'zen-boost:docs:ask';
    }
    ...
    public function getUsageLines(): array
    {
        return [
            ...
            'php zc_cli.php zen-boost:docs:ask <question>',
        ];
    }
    ...
    public function handle(ConsoleInput $input, ConsoleOutput $output): int
    {
        ...
        if ($question === '') {
            $output->errorln('Usage: bin/zencart zen-boost:docs:ask <question>');
            return 1;
        }

        $storage = $this->storage();
        $paths = $this->paths();
        $service = new \ZenBoostAnswerService();

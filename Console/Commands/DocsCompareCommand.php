...
class DocsCompareCommand extends AbstractZenBoostCommand
{
    public function getName(): string
    {
        return 'zen-boost:docs:compare';
    }
    ...
    public function getUsageLines(): array
    {
        return [
            ...
            'php zc_cli.php zen-boost:docs:compare <question>',
        ];
    }
    ...
    public function handle(ConsoleInput $input, ConsoleOutput $output): int
    {
        ...
        if ($query === '') {
            $output->errorln('Usage: bin/zencart zen-boost:docs:compare <question>');
            return 1;
        }

        $storage = $this->storage();
        $paths = $this->paths();
        $comparison = new \ZenBoostComparisonService();
